define([
    ...
    'ko',
    'mage/validation',
    'Prisma_Payway/js/view/payment/method-renderer/payway-method',
    ...
    return Component.extend({
        defaults: {
            template: 'Prisma_PaywayPromotions/payment/payway-promotions-payment',
            config: window.checkoutConfig.payment.payway,
            bankType: '',
            planInfo: ''
        },
        // Used to display exceptions during Payway API interactions
        ...
        getAvailableCards: function () {
            if (undefined != this.bankType()) {
                ...
                $('#payway_cc_type').find('option:not([value])').remove();
                $('#payway_cc_installment').find('option:not([value])').remove();
            }
        },
